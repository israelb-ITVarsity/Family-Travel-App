...
async function getCurrentUser(){
  const reuslts = await db.query("SELECT * FROM users");
  users = reuslts.rows;
...
app.post("/new", async (req, res) => {
  ...
  const addUser = await db.query("INSERT INTO users (user_name, colour) VALUES ($1, $2) RETURNING *;", [userName, userColor]);

  currentUserId= addUser.rows[0].id;
